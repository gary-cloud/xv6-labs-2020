...
// 和mappages一样, 只不过不再panic remapping, 直接强制复写
int
...
pagecopy(pagetable_t oldpage, pagetable_t newpage, uint64 begin, uint64 end) {
  ...
  for (i = begin; i < end; i += PGSIZE) {
    ...
    pa = PTE2PA(*pte);                      // 求得该页的物理地址
    flags = PTE_FLAGS(*pte) & (~PTE_U);     // 去掉 PTE_U 标志位
    // 建立 虚拟地址i 到 物理地址pa 大小PGSIZE 权限flags 的映射
    if (my_mappages(newpage, i, PGSIZE, pa, flags) < 0) {
      goto err;
    }
  }
  return 0;
